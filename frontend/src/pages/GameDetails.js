...
import { ArrowLeft, Trophy, Calendar, Users, Award, Ticket, Filter, CreditCard, MessageCircle, Copy, CheckCircle, Info, X, ZoomIn, ZoomOut } from 'lucide-react';
                  ...
                  {selectedPaymentMethod === 'upi' && (
                    <div className="space-y-4">
                      <div className="bg-[#1a1a2e] rounded-xl p-4 border border-white/10">
                        ...
                        {/* UPI App Buttons with Deep Links */}
                        <div className="space-y-3">
                          {/* Google Pay */}
                          <a
                            href={`gpay://upi/pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_PAYEE_NAME)}&am=${getTotalAmount()}&cu=INR&tn=${encodeURIComponent(bookingRequestId + ' - ' + (JSON.parse(localStorage.getItem('tambola_user') || '{}').name || 'Player'))}`}
                            className="flex items-center gap-4 w-full p-4 bg-white rounded-xl hover:bg-gray-100 transition-colors no-underline"
                          >
                            ...
                            <div className="flex-1">
                              <p className="text-gray-900 font-bold text-lg">Google Pay</p>
                              <p className="text-gray-500 text-sm">Pay ₹{getTotalAmount()}</p>
                            </div>
                          ...
                          {/* PhonePe */}
                          <a
                            href={`phonepe://pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_PAYEE_NAME)}&am=${getTotalAmount()}&cu=INR&tn=${encodeURIComponent(bookingRequestId + ' - ' + (JSON.parse(localStorage.getItem('tambola_user') || '{}').name || 'Player'))}`}
                            className="flex items-center gap-4 w-full p-4 bg-[#5f259f] rounded-xl hover:bg-[#4a1d7a] transition-colors no-underline"
                          >
                            <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center">
                              <svg viewBox="0 0 24 24" className="w-8 h-8">
                                <path fill="#5f259f" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15l-5-5 1.41-1.41L11 14.17l7.59-7.59L20 8l-9 9z"/>
                              </svg>
                            </div>
                            <div className="flex-1">
                              <p className="text-white font-bold text-lg">PhonePe</p>
                              <p className="text-purple-200 text-sm">Pay ₹{getTotalAmount()}</p>
                            </div>
                          ...
                          {/* Paytm */}
                          <a
                            href={`paytmmp://pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_PAYEE_NAME)}&am=${getTotalAmount()}&cu=INR&tn=${encodeURIComponent(bookingRequestId + ' - ' + (JSON.parse(localStorage.getItem('tambola_user') || '{}').name || 'Player'))}`}
                            className="flex items-center gap-4 w-full p-4 bg-[#00baf2] rounded-xl hover:bg-[#009ed1] transition-colors no-underline"
                          >
                            <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center">
                              <span className="text-[#00baf2] font-black text-sm">Paytm</span>
                            </div>
                            <div className="flex-1">
                              <p className="text-white font-bold text-lg">Paytm</p>
                              <p className="text-blue-100 text-sm">Pay ₹{getTotalAmount()}</p>
                            </div>
                        ...
                        <details className="mt-4">
                          <summary className="text-gray-400 text-sm cursor-pointer hover:text-white">
                            App not opening? Pay manually
                          </summary>
                          <div className="mt-3 space-y-2 p-3 bg-black/30 rounded-lg">
                            <div className="flex justify-between items-center">
                              <span className="text-gray-400 text-xs">UPI ID:</span>
                              <div className="flex items-center gap-2">
                                <span className="text-white font-mono text-sm">{UPI_ID}</span>
                                <Button variant="ghost" size="sm" onClick={copyUPIId} className="text-amber-500 hover:text-amber-400 h-6 px-1">
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-amber-500 font-bold">₹{getTotalAmount()}</span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-gray-400 text-xs">Reference:</span>
                              <span className="text-white font-mono text-xs">{bookingRequestId}</span>
                            </div>
                          </div>
                        </details>
                      </div>
                      ...
                      <div className="bg-[#1a1a2e] rounded-xl p-4 border border-white/10">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="bg-black/30 rounded-lg p-3">
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                <span className="text-amber-500 font-bold text-xl">${getConvertedAmount('interac').toFixed(2)} CAD</span>
                                <p className="text-gray-500 text-xs">(₹{getTotalAmount()} INR)</p>
                              </div>
                              <Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(getConvertedAmount('interac').toFixed(2)); toast.success('Amount copied!'); }} className="text-amber-500 hover:text-amber-400 h-8 px-2">
